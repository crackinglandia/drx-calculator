...
char szOnExec[] = "ONEXEC";
char szOnWrite[] = "ONWRITE";
char szOnRead[] = "ONREAD";
...
{
	MessageBox(hDlgMain, "DRx Calculator v1.1 x64\n\nCoded by +NCR/CRC! [ReVeRsEr]\n\nMail: crackinglandia(at)gmail(dot)com\nTwitter: @crackinglandia\n\nGeneral Pico, La Pampa\nArgentina\n\nSeptiembre 2011", "DRx Calculator x64", MB_ICONINFORMATION);
}

void ShowErrorObtainingHandles(HWND hDlgMain)
{
	MessageBox(hDlgMain, "Error obtaining handle! :(", "Ups!", MB_ICONERROR);
}
...
{
  InitCommonControls();
  DialogBoxParam(hInstance, (LPCTSTR)IDD_DIALOG1, 0, (DLGPROC)AppDlgProc, 0);
  return 0;
}